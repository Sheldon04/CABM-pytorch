                ...
                if self.args.test_patch:
                    ...
                    for lr, hr, filename in tqdm(d, ncols=80):
                        ...
                        for lr_sub_img, hr_sub_img in zip (lr_list, hr_list):
                            ...
                            time_end = time.time()
                            bit_config = util.get_bit_config(model)
                            lr_img =  util.tensor2img(lr_sub_img.detach().float().cpu())
                            imscore = util.laplacian(lr_img).mean()
                            imscore = round(imscore,2)
                            ...
                            if imscore not in self.config_dict.keys():
                                self.config_dict[imscore] = []
                                self.config_dict[imscore].append({'bit_config': bit_config, 'FAB': avg_bit})
                            else:
                                flag = True
                                for i, bt in enumerate(self.config_dict[imscore]):
                                    if operator.eq(bit_config, bt['bit_config']):
                                        flag = False
                                        break
                                if flag:
                                    self.config_dict[imscore].append({'bit_config': bit_config, 'FAB': avg_bit})
...
def main():
    if checkpoint.ok:
        ...
        while not t.terminate():
            t.test()
        dict_2 = {}
        for k,v in sorted(config_dict.items(), key=lambda x:float(x[0])):
            dict_2[k] = v
        for k,v in dict_2.items():
            v.sort(key=lambda x:x['FAB'], reverse=False)
        dict_json = json.dumps(dict_2)

        with open(f'{args.model}-{args.scale}-{args.data_test}.json','w') as f:
            f.write(dict_json)
        f.close()
        print('config saved!')
        checkpoint.done()
