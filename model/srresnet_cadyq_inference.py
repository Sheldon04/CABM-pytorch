...
class ResBlock_CADyQ(nn.Module):
    ...
    def forward(self, x):
        ...
        grad,residual,bits = self.classify1([grad, self.shortcut(x),bits]) 
        res = self.act(self.bn1(self.conv1(x))) 

        grad, res, bits = self.classify2([grad, res, bits])
        res = self.conv2(res)
        f1 = res
...
class SRResNet_CADyQ_I(nn.Module):
    def __init__(self, args, conv=quant_conv3x3, bias=False, k_bits =32 ):
        super(SRResNet_CADyQ_I, self).__init__()
