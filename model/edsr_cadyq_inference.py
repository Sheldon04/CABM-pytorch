...
class ResidualBlock_CADyQ(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.shortcut(x)
        tmp_bits = bits
        grad,x,bits = self.bitsel1([grad,x,bits]) # cadyq
        residual = x
        ...
        grad,x,bits= self.body[2]([grad,x,bits]) # cadyq
        ...
        return [grad, out, bits, f]




class EDSR_CADyQ_I(nn.Module):
    def __init__(self, args, conv=quant_conv3x3, bias = False, k_bits = 32): 
        super(EDSR_CADyQ_I, self).__init__()
    ...
    def forward(self, x):
        ...
        image_grad, res, bits, f = self.body[:-1]([image_grad, res, bits, f])
        res = self.body[-1](res)
