...
class DBlock_CADyQ(nn.Module):
    ...
    def forward(self, x):
        ...
        grad,x,bits= self.enhancement_top[0]([grad,x,bits]) # BitSelector
        
        x = self.enhancement_top[1:3](x)
        grad,x,bits = self.enhancement_top[3]([grad,x,bits])
        
        x = self.enhancement_top[4:6](x)
        grad,x,bits = self.enhancement_top[6]([grad,x,bits])
        ...
        grad,x,bits = self.enhancement_bottom[0]([grad,slice_1,bits])

        x = self.enhancement_bottom[1:3](x)
        grad,x,bits = self.enhancement_bottom[3]([grad,x,bits])

        x = self.enhancement_bottom[4:6](x)
        grad,x,bits = self.enhancement_bottom[6]([grad,x,bits])
