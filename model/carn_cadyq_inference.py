...
class ResidualBlock_CADyQ(nn.Module):
    ...
    def forward(self, x):
        ...
        residual = x
        grad,x,bits= self.body[0]([grad,x,bits]) # cadyq
        x = self.body[1:3](x) # conv-relu
        grad,x,bits= self.body[3]([grad,x,bits]) # cadyq
