...
from model.carn_cadyq_inference import CARN_CADyQ_I

from model.edsr_pams import EDSR_PAMS
from model.edsr_pams import EDSR_PAMS
from model.edsr_cadyq_inference import EDSR_CADyQ_I

from model.idn_pams import IDN_PAMS
from model.idn_cadyq_inference import IDN_CADyQ_I

from model.srresnet_pams import SRResNet_PAMS
from model.srresnet_cadyq_inference import SRResNet_CADyQ_I
...
class Trainer():
    def __init__(self, args, loader, t_model, s_model, ckp, config_dict):
        ...
        if args.model == 'EDSR' or args.model == 'SRResNet':
            arch_param = [v for k, v in self.s_model.named_parameters(
            ) if 'alpha' not in k and 'net' not in k]
            alpha_param = [
                v for k, v in self.s_model.named_parameters() if 'alpha' in k]
    ...
    def train(self):
        ...
        for batch, (lr, hr, idx_scale, ) in enumerate(self.loader_train):
            ...
            for i in range(lr.size(0)):
                lr_img = util.tensor2img(lr[i].detach().float().cpu())
                imscore=(util.laplacian(lr_img).mean())
                while str(round(imscore, args.select_float)) not in self.config_dict.keys() or self.config_dict[str(round(imscore, args.select_float))][0]["FAB"] >= args.calibration:
                    if args.select_float == 1:
                        imscore -= 0.1
                    elif args.select_float == 2:
                        imscore -= 0.01
                    elif args.select_float == 3:
                        imscore -= 0.001
                    imscore = -imscore if imscore < 0 else imscore
                imscore = str(round(imscore, args.select_float))
                # print(imscore)
                cindex = random.randint(0,len(self.config_dict[imscore])-1)
                cindex = cindex if args.select_bit == 2 else args.select_bit
                bit_config = self.config_dict[imscore][cindex]["bit_config"]
                flags.append(bit_config)
            
            util.set_bit_flag(self.s_model, flags)
            ...
            # Student
            if self.args.model == 'CARN':
                s_sr, s_res, bits, s_feat = self.s_model(lr/255., self.scale[0])
                s_sr *= 255.
            else:
                s_sr, s_res, bits, s_feat= self.s_model(lr)
                ...
                if self.args.test_patch:
                    ...
                    for lr, hr, filename in tqdm(d, ncols=80):
                        ...
                        for lr_sub_img, hr_sub_img in zip(lr_list, hr_list):
                            ...
                            flags = []
                            flags.append(bit_config)
                            util.set_bit_flag(model, flags)
                            time_start = time.time()
                            # --------------------select which quantization to pass through---------------------
                            if self.args.model == 'CARN':
                                sr_sub, _, bits, _ = model(
                    ...
                    self.ckp.write_log(
                            '[{} x{}] PSNR: {:.3f} SSIM:{:.3f} (Best PSNR: {:.3f} @epoch {}) {:.2f} bits'.format(
                                ...
                                best_psnr[1][idx_data, idx_scale] +
                                1 + self.resume_epoch,
                                tot_bits / len(d)
                            )
                        )
                    ...
                    for lr, hr, filename in pbar:
                        ...
                        if self.args.chop:
                            ...
                            if self.args.model.lower() == 'fsrcnn':
                                ...
                                sr, sr_res, bits, s_feat, s_w = model(
...
def main():
    if checkpoint.ok:
        ...
        if args.model == 'CARN':
            # teacher model (8-bit)
            t_model = CARN_PAMS(args, bias=True, k_bits=args.k_bits,
                                multi_scale=True, linq=False, fully=False).to(device)
            # student model
            s_model = CARN_CADyQ_I(
                args, bias=True, multi_scale=args.multi_scale).to(device)

        elif args.model == 'EDSR':
            t_model = EDSR_PAMS(args, bias=True, k_bits=args.k_bits).to(device)
            s_model = EDSR_CADyQ_I(args, bias=True, k_bits=args.k_bits).to(device)

        elif args.model == 'IDN':
            t_model = IDN_PAMS(args, bias=True, k_bits=args.k_bits).to(device)
            s_model = IDN_CADyQ_I(args, bias=True).to(device)

        elif args.model == 'SRResNet':
            t_model = SRResNet_PAMS(
                args, bias=True, k_bits=args.k_bits).to(device)
            s_model = SRResNet_CADyQ_I(
                args, bias=True, k_bits=args.k_bits).to(device)
        ...
        if args.teacher_weights is not None:
            ...
            # s_model.load_state_dict(t_checkpoint)
        ...
        config_dict = {}
        with open(f"{args.model}-{args.scale}-['div2k_valid'].json", "r") as f:
                config_dict = json.load(f) 
                f.close()
        t = Trainer(args, loader, t_model, s_model, checkpoint, config_dict)
